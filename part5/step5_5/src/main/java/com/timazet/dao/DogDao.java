...
public class DogDao {
    ...
    private static final Logger log = LoggerFactory.getLogger(DogDao.class);

    private final JdbcConnectionHolder connectionHolder;

    public DogDao(final JdbcConnectionHolder connectionHolder) {
        this.connectionHolder = connectionHolder;
    }

    @Log //weaving is possible even for final and private methods
    @PostConstruct
    private void init() {
    ...
    public Dog get(final UUID id) {
        return executeQuery("SELECT id, name, birth_date, height, weight FROM DOG WHERE id = ?", statement -> statement.setObject(1, id), resultSet -> {
    ...
    public Dog create(final Dog dog) {
        ...
        executeUpdate("INSERT INTO DOG (id, name, birth_date, height, weight) values (?, ?, ?, ?, ?)", statement -> {
    ...
    public Dog update(final Dog dog) {
        int count = executeUpdate("UPDATE DOG SET name = ?, birth_date = ?, height = ?, weight = ? where id = ?", statement -> {
    ...
    private Dog convertSingle(ResultSet resultSet) throws SQLException {
        ...
        result.setBirthDate(Optional.ofNullable(resultSet.getDate(Dog.BIRTH_DATE)).map(Date::toLocalDate).orElse(null));
